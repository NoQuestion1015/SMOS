...
def update_version(is_update):
    current_version = get_current_version()
    if current_version:
        major, minor = map(int, current_version.split('.'))
        if is_update:
            new_version = f"{major}.{minor + 1}"  # Увеличиваем минорную версию
        else:
            new_version = f"{major + 1}.0"  # Переход к следующей основной версии
...
def show_version_dialog():
    ...
    def on_update_click():
        ...
        change_type = "update"
        new_version = update_version(True)
        show_change_input()

    def on_new_version_click():
        nonlocal change_type, new_version
        change_type = "new"
        new_version = update_version(False)
        show_change_input()
